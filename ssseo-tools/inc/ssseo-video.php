 ...
 */
function ssseo_customize_register_youtube( WP_Customize_Manager $wp_customize ) {
    // Section: YouTube Integration
    $wp_customize->add_section( 'ssseo_youtube_section', array(
        'title'       => __( 'YouTube Integration', 'ssseo' ),
        'priority'    => 160,
        'description' => __( 'Enter your YouTube API credentials.', 'ssseo' ),
    ) );

    // Setting & Control: API Key
    $wp_customize->add_setting( 'ssseo_youtube_api_key', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
        'transport'         => 'refresh',
    ) );
    $wp_customize->add_control( 'ssseo_youtube_api_key', array(
        'label'       => __( 'YouTube API Key', 'ssseo' ),
        'section'     => 'ssseo_youtube_section',
        'type'        => 'text',
        'input_attrs' => array(
            'placeholder' => 'AIza…',
        ),
    ...
    // Setting & Control: Channel ID
    $wp_customize->add_setting( 'ssseo_youtube_channel_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
        'transport'         => 'refresh',
    ) );
    $wp_customize->add_control( 'ssseo_youtube_channel_id', array(
        'label'       => __( 'YouTube Channel ID', 'ssseo' ),
        'section'     => 'ssseo_youtube_section',
        'type'        => 'text',
        'input_attrs' => array(
            'placeholder' => 'UCXXXX…',
    ...
    // Setting & Control: Generate Video Posts (dummy setting to host a button)
    $wp_customize->add_setting( 'ssseo_generate_videos', array(
        'sanitize_callback' => 'sanitize_text_field',
        'capability'        => 'manage_options',
    ) );
    $wp_customize->add_control(
        new SSSEO_Generate_Videos_Control(
            $wp_customize,
            'ssseo_generate_videos',
            array(
                'section'  => 'ssseo_youtube_section',
                'settings' => 'ssseo_generate_videos',
                'label'    => __( 'Generate Video Posts', 'ssseo' ),
            )
        )
    );
}
add_action( 'customize_register', 'ssseo_customize_register_youtube' );

/**
 * Customizer Control: Button to trigger video post generation
 */
if ( class_exists( 'WP_Customize_Control' ) ) {
    class SSSEO_Generate_Videos_Control extends WP_Customize_Control {
        public $type = 'ssseo-generate-videos-button';

        public function render_content() {
            if ( ! current_user_can( 'manage_options' ) ) {
                echo '<p>' . esc_html__( 'You do not have permission to generate video posts.', 'ssseo' ) . '</p>';
                return;
            }
            $nonce = wp_create_nonce( 'ssseo_generate_videos_nonce' );
            ?>
            <label>
                <span class="customize-control-title"><?php esc_html_e( 'Generate Video Posts', 'ssseo' ); ?></span>
            </label>
            <button 
                type="button" 
                class="button button-primary ssseo-generate-videos-btn"
                data-nonce="<?php echo esc_attr( $nonce ); ?>"
            >
                <?php esc_html_e( 'Fetch & Create Drafts', 'ssseo' ); ?>
            </button>
            <p class="description">
                <?php esc_html_e( 'Click to fetch all videos from the channel and create Video CPT drafts (if they don’t already exist).', 'ssseo' ); ?>
            </p>
...
function ssseo_render_channel_page( $channel_id, $per_page = 4, $page = 1, $max_items = 0 ) {
    ...
    foreach ( $slice as $item ) {
        ...
        if ( ! empty( $current_video_id ) && $vid === $current_video_id ) {
            continue;
        }

        $full    = $item['snippet']['title'];
        $short   = esc_html( wp_trim_words( strip_tags( $full ), 6, '…' ) );
        $thumb   = esc_url( $item['snippet']['thumbnails']['medium']['url'] );
        $id_vid  = 'ytModal_' . $vid;
        $id_desc = 'ytDescModal_' . $vid;

        // Check if a Video CPT exists with slug == $vid
        ...
        // Visit Post button, if the CPT exists
        if ( $visit_url ) {
            printf(
        ...
        printf(
            '<h5 class="modal-title" id="%1$sLabel">%2$s</h5>',
            esc_attr( $id_vid ),
            $short
        );
        ...
        // Description Modal
        $desc = wp_kses_post( wpautop( esc_html( $item['snippet']['description'] ) ) );
        ...
        echo '        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="' . esc_attr__( 'Close', 'ssseo' ) . '"></button>';
        echo '      </div>'; // .modal-header
        echo '      <div class="modal-body">' . $desc . '</div>';
        echo '      <div class="modal-footer">';
        echo '        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">' . esc_html__( 'Close', 'ssseo' ) . '</button>';
        echo '      </div>'; // .modal-footer
        echo '    </div>'; // .modal-content
