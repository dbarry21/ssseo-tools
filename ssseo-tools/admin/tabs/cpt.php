<?php
/**
 * Admin UI: Enable CPT Tab (Service, Service Area, Product, Video)
 * – Bootstrap Toggle Version
 */
...
if (
    ...
    $fields = [
        ...
        'video'        => 'ssseo_enable_video_cpt',       // ← NEW
    ];
    ...
    <div class="col-md-4">
      <div class="card mb-4 shadow-sm">
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            <label for="<?php echo esc_attr($field_id); ?>_hasarchive" class="form-label">Has Archive</label>
            <input
              type="text"
              class="form-control"
              id="<?php echo esc_attr($field_id); ?>_hasarchive"
              name="<?php echo esc_attr($field_id); ?>_hasarchive"
              value="<?php echo esc_attr($s['has_archive']); ?>"
              placeholder="<?php echo $key === 'video' ? 'e.g. videos' : 'e.g. services'; ?>"
            >
            <div class="form-text">Leave blank to disable archive support.</div>
          </div>

          <div class="mb-3">
            <label for="<?php echo esc_attr($field_id); ?>_slug" class="form-label">Slug</label>
            <input
              type="text"
              class="form-control"
              id="<?php echo esc_attr($field_id); ?>_slug"
              name="<?php echo esc_attr($field_id); ?>_slug"
              value="<?php echo esc_attr($s['slug']); ?>"
              placeholder="<?php echo $key === 'video' ? 'e.g. video' : 'e.g. service'; ?>"
            >
            <div class="form-text">Enter the CPT URL base (no slashes). If blank, a sensible default is used.</div>
          </div>
          ...
          <?php if ($key === 'video'): ?>
            <div class="alert alert-info mb-0">
              <strong>Notes:</strong>
              <ul class="mb-0">
                <li>When bulk-creating YouTube drafts, each draft’s <em>post slug</em> = sanitized video title.</li>
                <li>Video ID is stored in meta <code>_ssseo_video_id</code>.</li>
              </ul>
            </div>
          <?php endif; ?>
