...
$default_pt = array_key_exists('video', $posts_by_type) ? 'video' : array_key_first($posts_by_type);

// Enqueue JS and pass data
add_action('admin_footer', function() use ($posts_by_type, $default_pt) {
    echo '<script>';
    echo 'var ssseoPostsByType = ' . wp_json_encode($posts_by_type) . ';';
    echo 'var ssseoDefaultType = "' . esc_js($default_pt) . '";';
    echo '</script>';
});
...
add_action('admin_enqueue_scripts', function($hook) {
    ...
    wp_enqueue_script(
        'ssseo-ai',
        plugin_dir_url(__DIR__) . '../../assets/js/ssseo-ai.js',
        ['jquery'],
        '1.0.0',
        true
    );
...
<a href="<?php echo esc_url(home_url('/llms.txt')); ?>" target="_blank" class="btn btn-outline-primary mt-3">View Full llms.txt in New Tab</a>
